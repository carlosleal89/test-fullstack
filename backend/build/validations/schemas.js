...
exports.createUserValidator = joi_1.default.object({
    name: joi_1.default.string().regex(/^[a-zA-Z\s]*$/)
        ...
        .min(4).message('"Nome" deve ter pelo menos 4 caracteres.').max(250).required(),
    email: joi_1.default.string().email({ tlds: { allow: false } })
        .message('"Email" incorreto.').required(),
    phone: joi_1.default.string().pattern(/^\d{2}\s\d{5}-\d{4}|\d{2}\s\d{4}-\d{4}$/)
